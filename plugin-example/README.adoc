...
3. Create a JAR file named `slackerdb-plugin-example-0.2.1.jar` in the `target` directory
...
2. Creates an additional JAR file with `-standalone` classifier: `slackerdb-plugin-example-0.2.1-standalone.jar`
...
java -jar slackerdb-plugin-example-0.2.1-standalone.jar
----
