...
....
    java -jar dbserver/target/slackerdb-dbserver-0.2.0-standalone.jar start
....

==== Stop db server

....
    java -jar dbserver/target/slackerdb-dbserver-0.2.0-standalone.jar stop
....

==== Check db status

....
    java -jar dbserver/target/slackerdb-dbserver-0.2.0-standalone.jar status
....

==== Start db proxy

....
    java -jar dbproxy/target/slackerdb-dbproxy-0.2.0-standalone.jar start
....

==== Stop db proxy

....
    java -jar dbproxy/target/slackerdb-dbproxy-0.2.0-standalone.jar stop
....

==== Check proxy status

....
    java -jar dbproxy/target/slackerdb-dbproxy-0.2.0-standalone.jar status
....
...
....
java -jar dbserver/target/slackerdb-dbserver-0.2.0-standalone.jar start --socket /tmp/.s.slackerdb.4309
....
...
----
+
The resulting JAR file (`plugin‑example‑0.2.0.jar`) will be placed in the `target` directory.
