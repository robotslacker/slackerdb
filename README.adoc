...
* You can use COPY syntax to import data with high performance, this
...
The `plugin‑example` module in the project provides a complete plugin example that demonstrates the basic structure and resource access methods. Developers can refer to this example to get started quickly.
...
* `plugid` – Plugin identifier (must match the JAR filename without the `.jar` extension). The plugin JAR must be located in the server's plugin directory (configured via `plugins_dir` parameter).
...
{
  "retCode": -1,
  "retMsg": "Plugin not found: myplugin"
}
----
