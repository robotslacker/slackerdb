...
以下HTTP端点提供独立于SQL REPL Server的基础管理功能，如备份、文件上传/下载、日志查看和状态监控。这些端点在数据服务API端口（port_x）启用时可用。
...
SlackerDB includes a web-based SQL console that provides an interactive interface for executing SQL queries, viewing results, and performing administrative tasks directly from your browser.

*Access:* After starting the server with data service API port (port_x) enabled, open `http://<server>:<port_x>/console.html` in a modern browser.
...
4. Use the sidebar buttons to open status, backup, or log modals.

The console is built with plain HTML/JavaScript and requires no additional installation. It is intended for development and administrative purposes in trusted environments.
