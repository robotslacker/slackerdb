...
import org.slackerdb.cdb.message.response.AdminClientResp;
import org.slackerdb.cdb.server.CDBInstance;
import org.slackerdb.cdb.server.ProxySession;
...
public class AdminClientRequest extends PostgresRequest {
    ...
    public AdminClientRequest(CDBInstance pCDBInstance) {
        super(pCDBInstance);
    }
    ...
    public void process(ChannelHandlerContext ctx, Object request) throws IOException {
        ...
        if (clientRequestCommand.trim().toUpperCase().startsWith("STOP"))
        {
            // 关闭代理服务
            try {
                this.CDBInstance.stop();
            } catch (ServerException e) {
                this.CDBInstance.logger.error("Error closing backend proxy", e);
            }
        ...
        else if (clientRequestCommand.trim().toUpperCase().startsWith("STATUS"))
        {
            ...
            // 显示当前服务状态
            feedBackMsg.append("SERVER STATUS: ").append(this.CDBInstance.instanceState).append("\n");
            ...
            if (this.CDBInstance.bootTime != null) {
                feedBackMsg.append("  Boot: ").append(this.CDBInstance.bootTime.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"))).append("\n");

                // 计算时间差
                Duration duration = Duration.between(this.CDBInstance.bootTime, currentTime);
            ...
            feedBackMsg.append(String.format("%-20s", "  Bind_Host:")).append(this.CDBInstance.serverConfiguration.getBindHost()).append("\n");
            feedBackMsg.append(String.format("%-20s", "  Port:")).append(this.CDBInstance.serverConfiguration.getPort()).append("\n");
            feedBackMsg.append(String.format("%-20s", "  Max_Workers:")).append(this.CDBInstance.serverConfiguration.getMax_Workers()).append("\n");
            feedBackMsg.append(String.format("%-20s", "  Log:")).append(this.CDBInstance.serverConfiguration.getLog()).append("\n");
            feedBackMsg.append(String.format("%-20s", "  Log_Level:")).append(this.CDBInstance.serverConfiguration.getLog_level().levelStr).append("\n");
            feedBackMsg.append(String.format("%-20s", "  Locale:")).append(this.CDBInstance.serverConfiguration.getLocale()).append("\n");
            feedBackMsg.append(String.format("%-20s", "  Client_TimeOut:")).append(this.CDBInstance.serverConfiguration.getClient_timeout()).append("\n");

            // 显示当前的代理会话情况
            feedBackMsg.append("SERVER SESSIONS: \n");
            feedBackMsg.append("  Total ").append(this.CDBInstance.proxySessions.size()).append(" clients connected.\n");
            for (Integer sessionId : this.CDBInstance.proxySessions.keySet())
            {
                ProxySession proxySession = this.CDBInstance.getSession(sessionId);
        ...
        AdminClientResp adminClientResp = new AdminClientResp(this.CDBInstance);
        adminClientResp.setReturnMsg(feedBackMsg.toString());
        adminClientResp.process(ctx, request, out);

        // 发送并刷新返回消息
        PostgresMessage.writeAndFlush(ctx, AdminClientResp.class.getSimpleName(), out, this.CDBInstance.logger);
        out.close();

        // 如果是独占模式, 则停止命令将同时停止程序运行
        if (clientRequestCommand.trim().toUpperCase().startsWith("STOP") && this.CDBInstance.isExclusiveMode())
        {
            System.exit(0);
        }
    }
