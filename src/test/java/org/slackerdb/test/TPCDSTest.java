    ...
    static void initAll() throws SQLException {
        ...
        Thread dbThread = new Thread(() -> {
            try {
                ...
                ServerConfiguration.setMax_network_workers(100);
...
//                Main.setLogLevel("TRACE");
    ...
    void runSQL(String name, String sql)  {
        ...
        for (int i=0;i<3;i++) {
            try {
                pgConn = DriverManager.getConnection(connectURL, "", "");
                break;
        ...
        if (pgConn == null)
        {
            throw new RuntimeException("[TEST] DB connect failed.");
        }
        try {
            pgConn.setAutoCommit(false);
